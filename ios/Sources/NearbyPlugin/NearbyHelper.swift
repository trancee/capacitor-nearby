...
extension Data {
    ...
    func hash(_ length: Int) -> Data {
        return hash().subdata(in: 0..<length)
    }
    ...
    func crc32() -> UInt32 {
        ...
        return ~(self.bytes.reduce(~UInt32(0), { crc, byte in
            (crc >> 8) ^ table32[(Int(crc) ^ Int(byte)) & 0xFF]
        }))
    }
}
...
extension EndpointID {
    ...
    static func fromUUID(_ uuid: UUID) -> EndpointID {
        let data = uuid.data.hash(ENDPOINT_ID_LENGTH).bytes
        return fromBytes(data)
    }

    static func fromBytes(_ bytes: [UInt8]) -> EndpointID {
        var endpointID: EndpointID = ""

        let data = bytes.data.hash(ENDPOINT_ID_LENGTH)

        for c in data {
            endpointID += String(kEndpointIdChars[Int(c) % kEndpointIdChars.count])
        }

        //        for i in 0..<ENDPOINT_ID_LENGTH {
        //            endpointID += String(kEndpointIdChars[Int(data[i]) % kEndpointIdChars.count])
        //        }
